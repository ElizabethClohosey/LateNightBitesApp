  ...
  $("#btnYelpRequest").on("click", function() {
    ...
    $.ajax({
      ...
      url:
        ...
        "&longitude=" +
        longitude +
        "&radius=20000&categories=restaurants&open_now=true&%20term=restaurants",
      method: "GET",
      headers: {
        Authorization:
      ...
      for (let index = 0; index < businessesArr.length; index++) {
        ...
        //category of food
        console.log(
          "category " + JSON.stringify(businessesArr[index].categories)
        );
        for (let i = 0; i < businessesArr[index].categories.length; i++) {
          console.log(
            "businessesArr[index].categories[i].title" +
              businessesArr[index].categories[i].title
          );
